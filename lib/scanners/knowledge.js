...
/**
 * Signal 2: No hardcoded secrets (0-4 pts, INVERTED)
 * Scan .md, .json, .js files in claudeRoot for secret patterns.
 * 4 points for clean, fewer if secrets found.
 * High impact — leaked secrets in config files are a real security risk.
 *
...
function checkNoHardcodedSecrets(claudeRoot) {
  ...
  try {
    ...
    // Scan failure — assume clean (4 points)
 ...
 * Medium impact — good practice but mostly passive documentation.
 */
...
/**
 * Signal 4: Plugin blocklist with reasons (0-3 pts)
 * Parse blocklist JSON and count entries with non-empty reason field.
 * Medium-high impact — documented blocklists prevent unsafe plugin usage.
 */
...
/**
 * Signal 7: Any security behavior (0-2 pts)
 * Awards points for having ANY intentional security practice.
 * Low direct impact — derivative meta-signal that just counts other signals.
 *
