...
// MCP server names that indicate a secrets manager
const SECRETS_MCP_NAMES = [
  'secret',
  'vault',
  'credential',
  'wiom-secrets',
...
function checkNoHardcodedSecrets(claudeRoot) {
  ...
  try {
    ...
    signal.points = filesWithSecrets === 0 ? 1 : 0;
    ...
    // Scan failure — assume clean (1 point)
...
/**
 * Signal 4: Secrets manager MCP (0-3 pts)
 * Check mcpServers keys in settings for secrets-manager names.
 */
function checkSecretsManagerMcp(settingsPath) {
  const signal = {
    name: 'Secrets manager MCP',
    value: 'No secrets manager MCP configured',
    points: 0,
    maxPoints: 3,
    recommendation: 'Configure a secrets manager MCP server (e.g., wiom-secrets, 1password, bitwarden, vault) for centralized credential access',
  };
  ...
  try {
    ...
    if (!mcpServers || typeof mcpServers !== 'object') return signal;

    const serverKeys = Object.keys(mcpServers);
    const matchedServers = serverKeys.filter(key => {
      const lowerKey = key.toLowerCase();
      return SECRETS_MCP_NAMES.some(name => lowerKey.includes(name));
    });

    const count = matchedServers.length;
    signal.value = `${count} secrets-related MCP server${count !== 1 ? 's' : ''} found`;

    if (count === 0)      signal.points = 0;
    else if (count === 1) signal.points = 2;
...
/**
 * Signal 5: Plugin blocklist with reasons (0-2 pts)
...
function checkAnySecurityBehavior(otherSignals) {
  const signal = {
    ...
    points: 0,
    maxPoints: 3,
    recommendation: 'Start with any security practice \u2014 create a security rules file, mention credentials in rules, or configure a secrets MCP server',
  };
  ...
  if (signal.points >= 2) {
    ...
    signal.recommendation = 'Broaden your security coverage \u2014 add credential references to rules, enable security plugins, or configure a secrets manager';
  }
...
function scan(paths) {
  ...
  const coreSignals = [
    ...
    checkSecretsManagerMcp(paths.settingsPath),
