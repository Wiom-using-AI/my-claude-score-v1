...
/**
 * Signal 1: Claude co-authored commits (0-5 pts)
...
function checkCoAuthoredCommits(gitScanRoots) {
  ...
  else if (totalCommits <= 20)   signal.points = 2;
  else if (totalCommits <= 50)   signal.points = 3;
  else if (totalCommits <= 100)  signal.points = 4;
...
/**
 * Signal 2: Commit message quality (0-3 pts)
...
function checkCommitMessageQuality(gitScanRoots) {
  ...
  // Bonus: if 60%+ are in ideal range, award 3 points
  if (idealPct >= 60) {
    signal.points = 3;
  } else {
...
/**
 * Signal 3: Semantic commit prefixes (0-2 pts)
...
function checkSemanticPrefixes(gitScanRoots) {
  ...
  if (pct === 0)        signal.points = 0;
  else if (pct <= 30)   signal.points = 1;
...
/**
 * Signal 4: Usage activity (0-5 pts)
...
/**
 * Signal 5: Model diversity (0-3 pts)
...
/**
 * Signal 6: Hooks configured (0-2 pts)
...
function checkHooksConfigured(settingsPath) {
  ...
  if (hookCount === 0) {
    ...
    // 2+ hook types OR 1 hook type with multiple matchers
    signal.points = 2;
  }
