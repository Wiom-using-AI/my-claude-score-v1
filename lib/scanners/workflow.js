...
// ---------------------------------------------------------------------------
// Signal 2: Agent depth (0-2 pts) — reduced from 0-3
// ---------------------------------------------------------------------------
function scanAgentDepth(paths) {
  ...
  if (files.length === 0) {
    ...
    recommendation = `Good detail at ${avgLines} avg lines. Consider adding example interactions and failure-mode guidance to push past 80 lines.`;
  } else {
...
// ---------------------------------------------------------------------------
// Signal 8: Workflow explorer bonus (0-2 pts) — NEW
// Awards points to users who engage with multiple discoverable
// workflow features (plans + todos). This helps casual users who
// naturally use Claude's built-in features score more fairly.
// ---------------------------------------------------------------------------
function scanWorkflowExplorer(otherSignals) {
  ...
  } else if (hasPlans || hasTodos) {
    signal.points = 1;
    signal.value = hasPlans ? 'Plans active, no todos' : 'Todos active, no plans';
