...
// Each entry is a category — we count unique categories matched.
 ...
 * Combines file count, word count, and domain keyword coverage
 * into a single graduated signal. Replaces the old triple-penalty
 * of separate file/depth/domain signals.
 *
 * Tiers:
 *   0 — No rules files
 *   1 — 1 file, under 100 words
 *   2 — 1+ files, 100-299 words
 *   3 — 2+ files, 300-499 words
 *   4 — 2+ files, 500-999 words
 *   5 — 3+ files, 500+ words, 3+ domain keywords
 *   6 — 3+ files, 1000+ words, 5+ domain keywords
 *   7 — 4+ files, 1000+ words, 7+ domain keywords
 */
function checkRulesQuality(rulesDir) {
  ...
  try {
    ...
    // Tiered scoring
    if (fileCount >= 4 && totalWords >= 1000 && domainHits >= 7) {
      signal.points = 7;
    } else if (fileCount >= 3 && totalWords >= 1000 && domainHits >= 5) {
      signal.points = 6;
    } else if (fileCount >= 3 && totalWords >= 500 && domainHits >= 3) {
      signal.points = 5;
    } else if (fileCount >= 2 && totalWords >= 500) {
      signal.points = 4;
    } else if (fileCount >= 2 && totalWords >= 300) {
      signal.points = 3;
    ...
    } else if (signal.points >= 4) {
      signal.recommendation = 'Add more domain-specific keywords (security, testing, deployment, architecture) and aim for 1000+ words total';
    } else if (signal.points >= 2) {
      signal.recommendation = 'Expand your rules files \u2014 add more files covering different aspects of your workflow (500+ words across 2+ files)';
    } else {
      signal.recommendation = 'Add more detail to your rules files \u2014 thorough rules (100+ words) give Claude much better context';
    }
  } catch {
 ...
 * Scan gitScanRoots for CLAUDE.md files at each repo root.
 * High impact — per-project context improves every interaction in that repo.
 */
function checkProjectClaudeMd(gitScanRoots) {
  ...
  try {
    ...
    for (const repoRoot of roots) {
      ...
      if (fileExistsAndNonEmpty(claudeMdPath)) {
        foundCount++;
      }
    }
    ...
    // Scan failure — leave at 0
  }

  if (signal.points < 2) {
    signal.recommendation =
      'Add CLAUDE.md files to your project repos for project-specific context and conventions';
  }
 ...
 * Parse settings.json and count meaningful customization keys.
 * Medium impact — counting keys present, not quality of configuration.
 */
function checkSettingsCustomization(settingsPath) {
  ...
  if (signal.points < 3) {
    signal.recommendation =
      'Customize ~/.claude/settings.json with hooks, MCP servers, allowed tools, or environment variables';
  }
 ...
 * Check for allowedTools, deniedTools, local settings files, and permission scope.
 */
function checkCustomPermissions(paths) {
  ...
  try {
    ...
    // Check main settings.json for allowedTools
    ...
    if (settings && typeof settings === 'object') {
      ...
      // Bonus: detailed permissions (both allow + deny configured)
      if (Array.isArray(settings.allowedTools) && settings.allowedTools.length > 0 &&
    ...
    // Check for any settings.local.json in scanned repos
  ...
  if (signal.points < 2) {
    signal.recommendation =
      'Configure allowedTools / deniedTools in settings.json to control Claude\'s permissions per-project';
  }
